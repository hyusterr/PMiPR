...
FileGraph::FileGraph(std::string path, int undirected, std::vector<char*>& index2node) {
    ...
    // update node2index first
    for (int i=0; i<index2node.size(); i++)
    {
        this->node2index.insert_key(index2node[i]);
    }

    if (undirected==-1)
        this->load_from_reverse_edge_list(path, index2node);
    else
        this->load_from_edge_list(path, index2node, undirected);
}
...
void FileGraph::load_from_reverse_edge_list(std::string path, std::vector<char*>& index2node) {
    ...
    for (int i=0; i<this->file_names.size(); i++)
    {
        ...
        for (; line != this->file_lines[i]; line++)
        {
            ...
            if (from_index == -1)
            {
                ...
                from_index = this->node2index.search_key(from_node);
                index2node.push_back(strdup(from_node));
            }
            ...
            if (to_index == -1)
            {
                ...
                to_index = this->node2index.search_key(to_node);
                index2node.push_back(strdup(to_node));
            }
    ...
    std::cout << "\t# of node:\t" << index2node.size() << std::endl;
}


void FileGraph::load_from_edge_list(std::string path, std::vector<char*>& index2node, int undirected) {
    this->load_file_status(path);
    std::cout << "Loading Lines:" << std::endl;
    FILE *fin;
    char from_node[256], to_node[256];
