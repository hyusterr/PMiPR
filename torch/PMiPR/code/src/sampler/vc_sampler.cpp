...
VCSampler::VCSampler(FileGraph* file_graph) {
     ...
     */
    this->node_size = file_graph->node2index.keys.size();
    ...
    vertex_distribution.resize(this->node_size, 0.0);
    vertex_uniform_distribution.resize(this->node_size, 0.0);
    context_uniform_distribution.resize(this->node_size, 0.0);
    neg_distribution.resize(this->node_size, 0.0);
...
long VCSampler::draw_an_edge() {
    return random_range(0, this->edge_size);
}

long VCSampler::draw_an_edge(long vertex_index) {
    return this->context_sampler.draw(vertex_index);
}

int VCSampler::has_context(long vertex_index, long context_index) {
    long offset = this->context_sampler.get_offset(vertex_index);
    long branch = this->context_sampler.get_branch(vertex_index);
    for (int i=0; i<branch; i++)
    {
        if (context_index==this->contexts[offset+i])
            return 1;
    }
    return 0;
}
